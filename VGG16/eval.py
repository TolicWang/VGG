...
vgg16 = VGG16(is_training=False,keep_prob=1)
with tf.Session() as sess:
    saver = tf.train.Saver()
    saver.restore(sess, tf.train.latest_checkpoint(FLAGS.model_save_path))
    x_train, y_train, _ = load_data_cifar('../data/cifar-100-python/test')
    correct_sum = 0
    for i in range(100):
        ...
        correct_sum += tf.reduce_sum(tf.cast(prediction, tf.float32))
    print('Acc on test dataset:',sess.run(correct_sum/10000))
